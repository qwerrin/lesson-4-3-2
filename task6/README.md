...
照合が通っても、機械が見ているのは文字列の一致である。コンソールに出した3件目の
URL をそのままアドレスバーに入れて開き、**このスクリプトが出す2項目が両方とも
画面で一致している**ことを確かめる。

| 確認する項目 | 期待値（`results.json` の3件目） | 画面のどこで見るか |
|---|---|---|
| URL | `https://www.youtube.com/watch?v=YEr-cz9pAHU` | アドレスバー |
| タイトル | `Python基礎コース 合併版｜Pythonの入門から基礎までを6時間で習得【プログラミング初心者はこの1本でOK】` | 動画のタイトル |

![表示したURLを開いたところ](docs/05-youtube.png)

課題の要件が「タイトルと URL を抽出して表示する」なので、目視の確認もその2つで閉じる。
片方だけ写っている画面は、**もう片方については何も言っていない**。
